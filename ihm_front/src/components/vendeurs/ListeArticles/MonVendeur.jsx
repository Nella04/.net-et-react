...
const handleCloseArticles = () => {
 ...
 const handleSubmitReview = async (review) => {
  ...
  useEffect(() => {
    axios.get(`https://localhost:7091/api/Vendeurs/VendeurArticlePayeParAcheteur${idAcheteurL}`)
      .then(response => {
            ...
            <motion.div whileHover={{ scale: 1.05 }}>
              <Card sx={{ backgroundColor: "#FFF8DC", boxShadow: 3, borderRadius: 3 }}>
                <CardContent>
                  ...
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    Nom de boutique:{vendor.boutique}
                  </Typography>
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    Nombre paiement fait:{vendor.nbrPayement}
                  </Typography>
                  <Rating
