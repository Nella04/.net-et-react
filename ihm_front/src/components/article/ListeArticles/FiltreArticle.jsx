...
const FiltreArticle = ({terme}) => {
  ...
  if (!terme) return true; // Si aucun terme, afficher tous les articles

  const termeMinuscule = terme.toLowerCase();
  
  return (
    article.nomArticle?.toLowerCase().includes(termeMinuscule) ||
    article.description?.toLowerCase().includes(termeMinuscule) ||
    article.codeBar?.toLowerCase().includes(termeMinuscule) ||
    article.marque?.toLowerCase().includes(termeMinuscule)||
    article.categorie?.toLowerCase().includes(termeMinuscule)
  );
});
              ...
              <Card sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, bgcolor: '#FFF8DC', boxShadow: 3 }}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                  <CardContent sx={{ pb: 0.2 }}>
                    <Typography variant="h6" color="#6A0DAD">
                        {highlight(article.nomArticle,terme)}
                        -{highlight(article.codeBar,terme)} </Typography>
                        {/* {tronquerText({highlight(article.description,terme)})} */}
                    <Typography variant="body2">{highlight(tronquerText(article.description),terme)}</Typography>
                    <Typography variant="body2">categorie:{highlight(article.categorie,terme)}</Typography>
                    <Typography variant="body2" color="text.secondary">Marque : {highlight(article.marque,terme)}</Typography>
                    <Typography variant="body1" color="#FFD700">Prix : {article.prix} Ar</Typography>
                    {/* <Typography variant="body1" color="#FFD700"> codebar : </Typography> */}
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', px: 2, pb:1, gap: 2 }}>
                    ...
                    <Commandeacheteur vendeur={article.vendeurId} codebar={article.codeBar} />
                  </Box>
                </Box>
              </Card>
            </Grid>
