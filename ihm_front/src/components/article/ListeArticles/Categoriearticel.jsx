...
// const CategorieArticle = ({ nomcategori }) => {
...
// useEffect(() => {
//     axios.get(`https://localhost:7091/api/Articles/ParCategorie/${nomcategori}`)
//       .then(response => {
...
const CategorieArticle = ({nomcategorie}) => {
  const [articles, setArticles] = useState([]);
  const [vendeurs, setVendeurs] = useState([]);
  const [loading, setLoading] = useState(true);
  const [loadingVendeurs, setLoadingVendeurs] = useState(true);
  ...
  useEffect(() => {
    axios.get(`https://localhost:7091/api/Articles/ParCategorie/${nomcategorie}`)
      .then(response => {
        setArticles(response.data);
        setLoading(false);
        //console.log(response);
  ...
  return <h1>Aucun article disponible dans cette catégorie</h1>;
}
